  ...
  if (req.method === 'POST') {
    ...
    req.on('end', function() {
      ...
      try {
        ...
        // ── Forward al CRM Digiactiva ─────────────────────────────────────
        var CRM_WEBHOOK = process.env.CRM_WEBHOOK_URL || 'https://digiactiva.com/api/external/whatsapp-event';
        var CRM_API_KEY = process.env.CRM_API_KEY || '';
        if (CRM_WEBHOOK && CRM_API_KEY) {
          var crmPayload = {
            workspace_slug: process.env.CRM_WORKSPACE_SLUG || 'pasta-al-vuelo',
            phone: '+' + msg.from,
            name: msg.from,
          ...
          }).then(function(r) { console.log('[CRM] Status:', r.status); })
